...
class Wav2Vec2RawEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([Wav2Vec2EncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class Wav2Vec2_with_exit_s1(Wav2Vec2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.s1_lm_head = nn.Linear(config.hidden_size, config.vocab_size)
    ...
    def add_exit(self, start_point, end_point):
        ...
        self.configuration_s1.num_hidden_layers = start_point
        self.configuration_s2.num_hidden_layers = end_point - start_point
        ...
        for k,v in self.wav2vec2.state_dict().items():
            ...
            for k_s2,v_v2 in self.wav2vec2_s2.state_dict().items():
                if k.split(".")[-3:] == k_s2.split(".")[-3:] and int(k.split(".")[2]) == int(k_s2.split(".")[1])+start_point:
    ...
    def forward(
        ...
        s1_hidden_states = self.s1_dropout(s1_hidden_states)
        s1_logits = self.s1_lm_head(s1_hidden_states)
...
class Wav2Vec2_with_exit_s2(Wav2Vec2PreTrainedModel):
    ...
    def add_exit(self, end_point):
        ...
        for k,v in self.wav2vec2.state_dict().items():
            for k_s2,v_v2 in self.wav2vec2_s2.state_dict().items():
                if k.split(".")[-3:] == k_s2.split(".")[-3:] and int(k.split(".")[2]) == int(k_s2.split(".")[1])+end_point:
                    dict_s2[k_s2] = dict_pre_trained[k]
...
class Wav2Vec2_with_exit(Wav2Vec2PreTrainedModel):
    ...
    def add_exit(self, placement):
        ...
        for k,v in self.wav2vec2.state_dict().items():
            ...
            for k_s2,v_v2 in self.wav2vec2_s2.state_dict().items():
                if k.split(".")[-3:] == k_s2.split(".")[-3:] and int(k.split(".")[2]) == int(k_s2.split(".")[1])+placement:
                    dict_s2[k_s2] = dict_pre_trained[k]
    ...
    def forward(
        ...
        return CausalLMOutput(
            loss=loss, logits=s1_logits, hidden_states=s1_outputs.hidden_states, attentions=s1_outputs.attentions
        )
