...
from networks import resnet_s1, posenet_s1, posenet_s2, backbone_s2, backbone_s3
...
class construct_net_backup(object):
    ...
    def construct_net_s1(self):
        if self.backbone == 'resnet':
            ...
            tokenizer = Wav2Vec2CTCTokenizer("/home/slzhang/projects/ETBA/Inference/src/recursive_placement/vocab.json",
...
class construct_net(object):
    def __init__(self, split_point_s1: int = 0, split_point_s2: int = 0, split_point_s3: int = 0, exit_type: bool = False,
    ...
    def construct_net_s2(self):
        if self.backbone == 'resnet' or self.backbone == 'posenet':
            return backbone_s2(layers=[3, 4, 23, 3],
                               split_point_s1=self.split_point_s1,
                               split_point_s2=self.split_point_s2, 
                               split_point_s3=self.split_point_s3
                               )
...
def model_export_func_backup(model_name, start_point, end_point, exit_type=False):
    ...
    #                     "/home/slzhang/projects/ETBA/Inference/src/recursive_placement/models/" + inst.backbone + "_s0.onnx",
    ...
    if inst.backbone == "resnet" or inst.backbone == "posenet" or inst.backbone == "Wav2Vec2":
        ...
        torch.onnx.export(s1_model, dummy_input1, 
                            "/home/slzhang/projects/ETBA/Inference/src/recursive_placement/models/" + inst.backbone + "_s1.onnx",
        ...
        torch.onnx.export(s2_model, dummy_input2,
                            "/home/slzhang/projects/ETBA/Inference/src/recursive_placement/models/" + inst.backbone + "_s2.onnx",
                        input_names=s2_input_names, output_names=s2_output_names,
                        verbose=False,dynamic_axes={
                                        'input': {0: 'batch_size'},
        ...
        torch.onnx.export(
            s1_model,
            s1_model_args,
            f="/home/slzhang/projects/ETBA/Inference/src/recursive_placement/models/" + inst.backbone + "_s1.onnx",
        ...
        torch.onnx.export(
            s2_model,
            s2_model_args,
            f="/home/slzhang/projects/ETBA/Inference/src/recursive_placement/models/" + inst.backbone + "_s2.onnx",
...
def model_export_func(model_name, split_point_s1, split_point_s2, split_point_s3):
