...
def model_split(layers, split_point_s1, split_point_s2, split_point_s3):
    s1_pre_layer = [0] * len(layers)
    s1_post_layer = [0] * len(layers)
    s2_layer = [0] * len(layers)
    s3_layer = [0] * len(layers)

    add_pos = 0
    for i in range(split_point_s1):
        while s1_pre_layer[add_pos] == layers[add_pos]:
            add_pos = add_pos + 1
        s1_pre_layer[add_pos] = s1_pre_layer[add_pos] + 1

    add_pos = 0
    for i in range(split_point_s2):
        while s1_post_layer[add_pos] == layers[add_pos]:
            add_pos = add_pos + 1
        s1_post_layer[add_pos] = s1_post_layer[add_pos] + 1

    add_pos = 0
    for i in range(split_point_s3):
        while s2_layer[add_pos] == layers[add_pos]:
            add_pos = add_pos + 1
        s2_layer[add_pos] = s2_layer[add_pos] + 1

    s3_layer = [i-j for i, j in zip(layers, s2_layer)]
    s2_layer = [i-j for i, j in zip(s2_layer, s1_post_layer)]
...
class backbone_init(nn.Module):
    def __init__(
        ...
        super(backbone_init, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        print(self.s1_pre_layer)
        print(self.s1_post_layer)
        print(self.s2_layer)
        print(self.s3_layer)

        self.pre_layer1 = self._make_layer(block, 64, self.s1_pre_layer[0], layer_idx=0)
        self.pre_layer2 = self._make_layer(block, 128, self.s1_pre_layer[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], layer_idx=1)
        self.pre_layer3 = self._make_layer(block, 256, self.s1_pre_layer[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], layer_idx=2)
        self.pre_layer4 = self._make_layer(block, 512, self.s1_pre_layer[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], layer_idx=3)

        self.post_layer1 = self._make_layer(block, 64, self.s1_post_layer[0], start_of_stage=(self.s1_pre_layer[0]==0), layer_idx=0)
        self.post_layer2 = self._make_layer(block, 128, self.s1_post_layer[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], 
                                       start_of_stage=(self.s1_pre_layer[1]==0), layer_idx=1)
        self.post_layer3 = self._make_layer(block, 256, self.s1_post_layer[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], 
                                       start_of_stage=(self.s1_pre_layer[2]==0), layer_idx=2)
        self.post_layer4 = self._make_layer(block, 512, self.s1_post_layer[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], 
                                       start_of_stage=(self.s1_pre_layer[3]==0), layer_idx=3)
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False, start_of_stage: bool = True,
                    layer_idx: int = 0) -> nn.Sequential:
        self.inplanes_list = [64, 256, 512, 1024]
        if blocks == 0:
            layers = []
            return nn.Sequential(*layers)
        norm_layer = self._norm_layer
        ...
        if not start_of_stage:
            ...
            downsample = nn.Sequential(
                conv1x1(self.inplanes_list[layer_idx], planes * block.expansion, stride),
                norm_layer(planes * block.expansion),
            )
            layers.append(block(self.inplanes_list[layer_idx], planes, stride, downsample, self.groups,
                        self.base_width, previous_dilation, norm_layer))
            for _ in range(1, blocks):
                layers.append(block(planes * block.expansion, planes, groups=self.groups,
                                    base_width=self.base_width, dilation=self.dilation,
...
class backbone_s1(nn.Module):
    def __init__(
        ...
        self.pre_layer1 = self._make_layer(block, 64, self.s1_pre_layer[0], start_of_stage=(layers[0]==c_layer[0]), layer_idx=0)
        self.pre_layer2 = self._make_layer(block, 128, self.s1_pre_layer[1], stride=2, start_of_stage=(layers[1]==c_layer[1]),
                                       dilate=replace_stride_with_dilation[0], layer_idx=1)
        self.pre_layer3 = self._make_layer(block, 256, self.s1_pre_layer[2], stride=2, start_of_stage=(layers[2]==c_layer[2]),
                                       dilate=replace_stride_with_dilation[1], layer_idx=2)
        self.pre_layer4 = self._make_layer(block, 512, self.s1_pre_layer[3], stride=2, start_of_stage=(layers[3]==c_layer[3]),
                                       dilate=replace_stride_with_dilation[2], layer_idx=3)

        self.post_layer1 = self._make_layer(block, 64, self.s1_post_layer[0], 
                                       start_of_stage=(self.s1_pre_layer[0]==0 & layers[0]==c_layer[0]), layer_idx=0)
        self.post_layer2 = self._make_layer(block, 128, self.s1_post_layer[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], 
                                       start_of_stage=(self.s1_pre_layer[1]==0 & layers[1]==c_layer[1]), layer_idx=1)
        self.post_layer3 = self._make_layer(block, 256, self.s1_post_layer[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], 
                                       start_of_stage=(self.s1_pre_layer[2]==0 & layers[2]==c_layer[2]), layer_idx=2)
        self.post_layer4 = self._make_layer(block, 512, self.s1_post_layer[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], 
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False, start_of_stage: bool = True,
                    layer_idx: int = 0) -> nn.Sequential:
        self.inplanes_list = [128, 256, 512, 1024]
        if blocks == 0:
            layers = []
            return nn.Sequential(*layers)
        norm_layer = self._norm_layer
...
class backbone_s2(nn.Module):
    def __init__(
        ...
        self.s23_layer = [i+j for i, j in zip(self.s2_layer, self.s3_layer)]
        self.layer1 = self._make_layer(block, 64, self.s2_layer[0], 
                                       start_of_stage=(self.s23_layer[0]==c_layer[0]), layer_idx=0)
        self.layer2 = self._make_layer(block, 128, self.s2_layer[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], 
                                       start_of_stage=(self.s23_layer[1]==c_layer[1]), layer_idx=1)
        self.layer3 = self._make_layer(block, 256, self.s2_layer[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], 
                                       start_of_stage=(self.s23_layer[2]==c_layer[2]), layer_idx=2)
        self.layer4 = self._make_layer(block, 512, self.s2_layer[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], 
                                       start_of_stage=(self.s23_layer[3]==c_layer[3]), layer_idx=3)
...
class backbone_s3(nn.Module):
    def __init__(
        ...
        super(backbone_s3, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        self.layer1 = self._make_layer(block, 64, self.s3_layer[0], 
                                       start_of_stage=(self.s3_layer[0]==c_layer[0]), layer_idx=0)
        self.layer2 = self._make_layer(block, 128, self.s3_layer[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], 
                                       start_of_stage=(self.s3_layer[1]==c_layer[1]), layer_idx=1)
        self.layer3 = self._make_layer(block, 256, self.s3_layer[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], 
                                       start_of_stage=(self.s3_layer[2]==c_layer[2]), layer_idx=2)
        self.layer4 = self._make_layer(block, 512, self.s3_layer[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], 
                                       start_of_stage=(self.s3_layer[3]==c_layer[3]), layer_idx=3)
...
class resnet_s1(nn.Module):
    def __init__(
        ...
        super(resnet_s1, self).__init__()
        if is_init:
            self.backbone = backbone_init(layers=layers, 
    ...
    def forward(self, x):
        x_c, x_exit = self.backbone(x)
        x = self.avgpool(x_exit)
        x = torch.flatten(x, 1)
        x = self.fc(x)
...
class posenet_s1(nn.Module):
    def __init__(
        ...
        self.split_point_s1 = split_point_s1
        self.split_point_s2 = split_point_s2
        self.split_point_s3 = split_point_s3
        if is_init:
            self.backbone = backbone_init(layers=layers, 
                                        num_classes=num_classes, 
                                        split_point_s1=split_point_s1, 
...
class posenet_s1_backup(nn.Module):
...
if __name__ == '__main__':
    print("part1")
    part1_s1 = backbone_init(split_point_s1=1, split_point_s2=3, split_point_s3=6)
    part1_s2 = backbone_s2(split_point_s1=1, split_point_s2=3, split_point_s3=6)

    print("part2")
    s1_pre_layer, s1_post_layer, s2_layer, s3_layer = model_split(layers=[3,4,23,3],
                             split_point_s1=1, split_point_s2=3, split_point_s3=6)
    print("layer_s3" + str(s3_layer))
    part2_s1 = backbone_s1(layers=s3_layer, split_point_s1=7, split_point_s2=8, split_point_s3=10)
    part2_s2 = backbone_s2(layers=s3_layer, split_point_s1=7, split_point_s2=8, split_point_s3=10)

    print("part3")
    s1_pre_layer, s1_post_layer, s2_layer, s3_layer_ = model_split(layers=s3_layer,
                             split_point_s1=7, split_point_s2=8, split_point_s3=10)
    print("layer_s3" + str(s3_layer_))
    part3_s1 = backbone_s1(layers=s3_layer_, split_point_s1=2, split_point_s2=2, split_point_s3=4)
    part3_s2 = backbone_s2(layers=s3_layer_, split_point_s1=2, split_point_s2=2, split_point_s3=4) 
