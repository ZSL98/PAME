...
class backbone_s1(nn.Module):
    def __init__(
        ...
        # print(bn_type)
        # a = ModuleHelper.BatchNorm2d(bn_type=bn_type)(64)
        ...
        # multi_grid=None
        if dilate_scale == 8:
            self.pre_layer3.apply(partial(self._nostride_dilate, dilate=2))
            if multi_grid is None:
                self.pre_layer4.apply(partial(self._nostride_dilate, dilate=4))
            else:
                for i in range(len(self.pre_layer4)):
                    tmp = 4*multi_grid[i]
                    self.pre_layer4[i].apply(
                        partial(self._nostride_dilate, dilate=tmp)
                    )
    ...
    def _make_complex_exit(self, layer: int, stride: int = 1, bn_type=None) -> nn.Sequential:
        layers = []
        previous_dilation = self.dilation
        if layer < 1:
            downsample_1 = nn.Sequential(
            ...
            layers.append(Bottleneck(128, 64, stride, downsample_1, bn_type=bn_type))
        ...
        if layer < 2:
            ...
            layers.append(Bottleneck(256, 128, stride, downsample_2, bn_type=bn_type))

        if layer < 3:
            downsample_3 = nn.Sequential(
                conv1x1(512, 1024, stride),
                ModuleHelper.BatchNorm2d(bn_type=bn_type)(1024),
            )
            layers.append(Bottleneck(512, 256, stride, downsample_3, bn_type=bn_type))
        ...
        if layer < 4:
            ...
            layers.append(Bottleneck(1024, 512, stride, downsample_4, bn_type=bn_type))
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False, pre_or_not: bool = True,
                    layer_idx: int = 0, bn_type=None) -> nn.Sequential:
        if blocks == 0:
            layers = []
            return nn.Sequential(*layers)
        downsample = None
        ...
        if pre_or_not:
            ...
            if self.pre_layer[layer_idx] == 0:
                ...
                downsample = nn.Sequential(
                    conv1x1(self.inplanes, planes * block.expansion, stride),
                    ModuleHelper.BatchNorm2d(planes * block.expansion),
                )
                layers.append(block(self.inplanes, planes, stride, downsample, bn_type=bn_type))
            self.inplanes = planes * block.expansion
            for _ in range(0, blocks):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        # pre-network
        x_fork = self.pre_layer1(x)
        tuple_features.append(x_fork)
        x_fork = self.pre_layer2(x_fork)
        tuple_features.append(x_fork)
        x_fork = self.pre_layer3(x_fork)
        tuple_features.append(x_fork)

        if len(self.pre_layer4) > 0:
            x_fork = self.pre_layer4(x_fork)
            tuple_features.append(x_fork)
        # exit branch
        else:
            x_exit = self.exit(x_fork)
            tuple_features.append(x_exit)
...
class backbone_s2(nn.Module):
    def __init__(
        ...
        self.s1_layer = [i+j for i, j in zip(_pre_layer, _post_layer)]
        self.layer1 = self._make_layer(block, 64, self.s2_layer[0], layer_idx=0)
        self.layer2 = self._make_layer(block, 128, self.s2_layer[1], stride=2, layer_idx=1)
        self.layer3 = self._make_layer(block, 256, self.s2_layer[2], stride=2, layer_idx=2)
        self.layer4 = self._make_layer(block, 512, self.s2_layer[3], stride=2, layer_idx=3)
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
                    stride: int = 1, dilate: bool = False, layer_idx: int = 0, bn_type=None) -> nn.Sequential:
        if blocks == 0:
            layers = []
            return nn.Sequential(*layers)
        downsample = None
        ...
        self.inplanes = planes * block.expansion
        for _ in range(0, blocks):
            layers.append(block(self.inplanes, planes, stride, bn_type=bn_type))
