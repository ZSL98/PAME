...
class FSAuxCELoss_with_exit(nn.Module):
    def __init__(self, configer=None):
        super(FSAuxCELoss_with_exit, self).__init__()
        self.configer = configer
        self.ce_loss = FSCELoss(self.configer)

    def forward(self, inputs, targets, **kwargs):
        aux_out_s1, seg_out_s1, aux_out_s2, seg_out_s2 = inputs
        seg_loss_s1 = self.ce_loss(seg_out_s1, targets)
        aux_loss_s1 = self.ce_loss(aux_out_s1, targets)
        loss_s1 = self.configer.get("network", "loss_weights")["seg_loss"] * seg_loss_s1
        loss_s1 = (
            loss_s1 + self.configer.get("network", "loss_weights")["aux_loss"] * aux_loss_s1
        )
        seg_loss_s2 = self.ce_loss(seg_out_s2, targets)
        aux_loss_s2 = self.ce_loss(aux_out_s2, targets)
        loss_s2 = self.configer.get("network", "loss_weights")["seg_loss"] * seg_loss_s2
        loss_s2 = (
            loss_s2 + self.configer.get("network", "loss_weights")["aux_loss"] * aux_loss_s2
        )
