...
for exit in range(10,33,3):
    ori_hist_data = []
    if dataset in ['cola', 'mnli', 'mrpc', 'qnli', 'qqp', 'rte', 'sst2', 'stsb', 'wnli']:
        with open('./moveon_dict/bert/{}/{}_exit_l[0, 6, {}]_b{}_t{}.json'.format(dataset, dataset, exit, batch_size, metric_thres), 'rb') as f:
            last_moveon_dict = json.load(f)
    elif dataset in ['imagenet', 'imagenette', 'imagewoof']:
        with open('./moveon_dict/resnet/{}/resnet_exit_l[0, {}]_b{}_t{}_nette.json'.format(dataset, exit, batch_size, metric_thres), 'rb') as f:
            last_moveon_dict = json.load(f)
    else:
    ...
    hist_data = plt.hist(ori_hist_data, bins=4, range=(0,batch_size))
