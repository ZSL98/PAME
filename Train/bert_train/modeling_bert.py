...
class BertWithExit_s1(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def add_exit(self, start_point, end_point):
        ...
        for k,v in self.bert.state_dict().items():
            ...
            for k_s2,v_s2 in self.bert_s2.state_dict().items():
                if k.split(".")[3:] == k_s2.split(".")[2:] and int(k.split(".")[2]) == int(k_s2.split(".")[1])+start_point:
...
class BertWithExit_s2(BertPreTrainedModel):
    ...
    def add_exit(self, start_point, end_point):
        ...
        for k,v in self.bert.state_dict().items():
            for k_s2,v_s2 in self.bert_s2.state_dict().items():
                if k.split(".")[3:] == k_s2.split(".")[2:] and int(k.split(".")[2]) == int(k_s2.split(".")[1])+end_point:
                    dict_s2[k_s2] = dict_pre_trained[k]
...
class BertWithExit(BertPreTrainedModel):
    ...
    def add_exit(self, placement):
        ...
        for k,v in self.bert.state_dict().items():
            ...
            for k_s2,v_s2 in self.bert_s2.state_dict().items():
                if k.split(".")[3:] == k_s2.split(".")[2:] and int(k.split(".")[2]) == int(k_s2.split(".")[1])+placement:
                    dict_s2[k_s2] = dict_pre_trained[k]
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                if self.num_labels == 1:
                    s1_loss = loss_fct(s1_logits.squeeze(), labels.squeeze())
                    s2_loss = loss_fct(s2_logits.squeeze(), labels.squeeze())
                else:
                    s1_loss = loss_fct(s1_logits, labels)
                    s2_loss = loss_fct(s2_logits, labels)
            elif self.config.problem_type == "single_label_classification":
                loss_fct = CrossEntropyLoss()
                s1_loss = loss_fct(s1_logits.view(-1, self.num_labels), labels.view(-1))
                s2_loss = loss_fct(s2_logits.view(-1, self.num_labels), labels.view(-1))
            elif self.config.problem_type == "multi_label_classification":
                loss_fct = BCEWithLogitsLoss()
                s1_loss = loss_fct(s1_logits, labels)
                s2_loss = loss_fct(s2_logits, labels)
